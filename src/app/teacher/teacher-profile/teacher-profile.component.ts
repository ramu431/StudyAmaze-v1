...
export class TeacherProfileComponent implements OnInit {
  ...
  dob: any;
  userIdFk: any;
  constructor(private _location:Location, private teacher:TeacherService,private snackbar:MatSnackBar) { }

  ngOnInit(): void {
    let userId=sessionStorage.getItem('userid');
 
    this.teacher.teacherProfile(userId).subscribe(
      data=>{
        console.log(data);
  ...
  uploadPic(){
    if (this.fileList.length > 0){
      ...
      // }
        ...
        this.teacher.uploadProfilePic(formData).subscribe(
          res =>{
            console.log(res);
            this.upRes=res;
  ...
  console.log(newabout);
    this.teacher.SaveOrUpdate(newabout).subscribe(
      (data:any)=>
      {
        console.log(data);
        if(data){
